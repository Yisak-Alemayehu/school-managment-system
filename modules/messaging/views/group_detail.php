        ...
        <div class="bg-white dark:bg-dark-card rounded-b-xl border border-gray-200 dark:border-dark-border border-t-0 p-3">
            ...
            <form method="POST" action="<?= url('messaging', 'group-send') ?>" enctype="multipart/form-data" class="flex items-end gap-2">
                ...
                <label class="cursor-pointer text-gray-400 dark:text-gray-500 hover:text-green-600 flex-shrink-0 pb-2" title="Attach file">
                    ...
                    <input type="file" name="attachments[]" multiple class="hidden" id="group-reply-file-input" accept=".jpg,.jpeg,.png,.gif,.webp,.pdf,.doc,.docx,.xls,.xlsx,.webm,.ogg,.mp3,.wav">
                </label>
                <button type="button" id="group-record-btn" class="flex-shrink-0 p-2 text-gray-400 dark:text-gray-500 hover:text-green-600" title="Record voice message">
                    <span id="group-record-icon" class="text-lg">⏺</span>
                </button>
...
(function() {
    ...
    // Voice recording for group reply
    var groupRecordBtn = document.getElementById('group-record-btn');
    var groupRecordIcon = document.getElementById('group-record-icon');
    var groupMediaRecorder = null;
    var groupAudioChunks = [];

    function groupAddFileToInput(file) {
        if (!grpFileInput) return;
        var dt = new DataTransfer();
        Array.from(grpFileInput.files).forEach(function(f) { dt.items.add(f); });
        dt.items.add(file);
        grpFileInput.files = dt.files;
        grpFileInput.dispatchEvent(new Event('change'));
    }

    if (groupRecordBtn) {
        groupRecordBtn.addEventListener('click', function() {
            ...
            groupRecordBtn.classList.add('text-red-600');
            groupRecordIcon.textContent = '⏹';

            navigator.mediaDevices.getUserMedia({ audio: true }).then(function(stream) {
                groupMediaRecorder = new MediaRecorder(stream);
                groupAudioChunks = [];
                groupMediaRecorder.ondataavailable = function(e) {
                    if (e.data && e.data.size) groupAudioChunks.push(e.data);
                };
                groupMediaRecorder.onstop = function() {
                    stream.getTracks().forEach(function(t) { t.stop(); });
                    var blob = new Blob(groupAudioChunks, { type: 'audio/webm' });
                    var file = new File([blob], 'voice_' + Date.now() + '.webm', { type: blob.type });
                    groupAddFileToInput(file);

                    groupRecordBtn.classList.remove('text-red-600');
                    groupRecordIcon.textContent = '⏺';
                };
