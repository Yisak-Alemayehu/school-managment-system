    ...
    <form method="POST" action="<?= url('messaging', 'send') ?>" enctype="multipart/form-data"
        ...
        <div>
            ...
            <div class="relative">
                <input type="text" id="recipient-search" autocomplete="off"
                       ...
                       class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 <?= !empty($errors['recipient_id']) ? 'border-red-300' : '' ?>">
                <input type="hidden" name="recipient_id" id="recipient-id" value="<?= $recipientUser ? $recipientUser['id'] : old('recipient_id') ?>">
                <div id="recipient-dropdown" class="hidden absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-48 overflow-y-auto"></div>
            </div>
        ...
        <div>
            ...
            <textarea name="body" rows="6" required maxlength="5000"
                      class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 <?= !empty($errors['body']) ? 'border-red-300' : '' ?>"
                      placeholder="Type your message…"><?= old('body') ?></textarea>
            <?php if (!empty($errors['body'])): ?>
            <p class="mt-1 text-xs text-red-600"><?= e($errors['body']) ?></p>
            <?php endif; ?>
            <p class="mt-1 text-xs text-gray-400"><span id="char-count">0</span>/5000</p>
        </div>

        <!-- File Attachments -->
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Attachments <span class="text-gray-400">(max 5 files, 10 MB each)</span></label>
            ...
            <p class="mt-1 text-xs text-gray-400">Allowed: Images, PDF, Word, Excel</p>
            <div id="compose-file-preview" class="mt-2 flex flex-wrap gap-2"></div>
        </div>

        <!-- Submit -->
        <div class="flex items-center justify-end gap-3 pt-2">
            <a href="<?= url('messaging', 'inbox') ?>" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg text-sm hover:bg-gray-200 font-medium">Cancel</a>
        ...
        debounceTimer = setTimeout(function() {
            fetch('<?= url('messaging', 'api-search-users') ?>&q=' + encodeURIComponent(q))
                ...
                .then(function(data) {
                    ...
                    if (!users.length) {
                        dropdown.innerHTML = '<div class="px-3 py-2 text-sm text-gray-500">No users found</div>';
                    } else {
                        dropdown.innerHTML = users.map(function(u) {
                            return '<div class="px-3 py-2 text-sm hover:bg-gray-100 cursor-pointer flex items-center gap-2" data-id="' + u.id + '" data-name="' + (u.full_name || '').replace(/"/g, '&quot;') + '">'
                                ...
                                + '<div class="text-xs text-gray-400">' + (u.role || '') + '</div></div></div>';
                        }).join('');
                    }
            ...
            Array.from(this.files).forEach(function(file) {
                ...
                if (file.type.startsWith('image/')) {
                    ...
                    reader.onload = function(e) {
                        item.innerHTML = '<img src="' + e.target.result + '" class="w-20 h-20 object-cover rounded-lg border border-gray-200">'
                            + '<p class="text-[10px] text-gray-500 mt-0.5 truncate max-w-[80px]">' + escHtml(file.name) + '</p>';
                    };
                    reader.readAsDataURL(file);
                } else {
                    var ext = file.name.split('.').pop().toUpperCase();
                    var size = (file.size / 1024).toFixed(0) + ' KB';
                    item.innerHTML = '<div class="w-20 h-20 rounded-lg border border-gray-200 bg-gray-50 flex flex-col items-center justify-center">'
