...
<div class="space-y-4">
    <div class="flex items-center justify-between">
        <h1 class="text-xl font-bold text-gray-900">My Groups</h1>
    ...
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <a href="<?= url('messaging', 'group-detail', $g['id']) ?>"
           class="bg-white rounded-xl border border-gray-200 p-4 hover:border-green-300 hover:shadow-sm transition-all">
            <div class="flex items-start gap-3">
                <div class="w-10 h-10 rounded-full bg-green-100 flex items-center justify-center flex-shrink-0">
                    <svg class="w-5 h-5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                </div>
                <div class="min-w-0 flex-1">
                    <h3 class="text-sm font-semibold text-gray-900 truncate"><?= e($g['name']) ?></h3>
                    <?php if ($g['description']): ?>
                    <p class="text-xs text-gray-500 mt-0.5 truncate"><?= e($g['description']) ?></p>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <?php if ($g['class_name']): ?>
            <p class="text-xs text-gray-400 mt-1"><?= e($g['class_name']) ?><?= $g['section_name'] ? ' - ' . e($g['section_name']) : '' ?></p>
            <?php endif; ?>
        </a>
        <?php endforeach; ?>
    </div>
