...
<div class="max-w-2xl mx-auto space-y-4">
    <div class="flex items-center gap-3">
        ...
        <h1 class="text-xl font-bold text-gray-900 dark:text-dark-text">Create Group</h1>
    </div>
    ...
    <form method="POST" action="<?= url('messaging', 'group-create') ?>" class="bg-white dark:bg-dark-card rounded-xl border border-gray-200 dark:border-dark-border p-6 space-y-4">
        <?= csrf_field() ?>
        ...
        <!-- Classmate Selector -->
        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Add Members (max 30)</label>
            <?php if (empty($classmates)): ?>
            <p class="text-sm text-gray-500 dark:text-dark-muted">No classmates found in your class/section.</p>
            <?php else: ?>
            <div class="border border-gray-300 dark:border-dark-border rounded-lg max-h-60 overflow-y-auto p-2 space-y-1">
                <?php foreach ($classmates as $cm): ?>
                <label class="flex items-center gap-2 px-2 py-1.5 hover:bg-gray-50 dark:bg-dark-bg rounded cursor-pointer">
            ...
            <p class="mt-1 text-xs text-gray-400 dark:text-gray-500"><?= count($classmates) ?> classmate(s) available</p>
            <?php endif; ?>
        </div>

        <div class="flex items-center justify-end gap-3 pt-2">
            <a href="<?= url('messaging', 'groups') ?>" class="px-4 py-2 bg-gray-100 dark:bg-dark-card2 text-gray-700 dark:text-gray-300 rounded-lg text-sm hover:bg-gray-200 font-medium">Cancel</a>
