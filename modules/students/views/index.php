...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <div>
        <h1 class="text-xl font-bold text-gray-900 dark:text-dark-text">Students</h1>
        <p class="text-sm text-gray-500 dark:text-dark-muted"><?= number_format($totalStudents) ?> students found</p>
    </div>
    <div class="flex gap-2">
        ...
        <?php endif; ?>
        <a href="<?= url('students', 'export') ?>&class_id=<?= $classFilter ?>&section_id=<?= $sectionFilter ?>" class="inline-flex items-center gap-2 px-4 py-2 border border-gray-300 dark:border-dark-border rounded-lg text-sm bg-white dark:bg-dark-card dark:text-dark-text text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:bg-dark-bg">
...
<div class="bg-white dark:bg-dark-card rounded-xl border border-gray-200 dark:border-dark-border p-4 mb-4">
    <form method="GET" action="<?= url('students') ?>" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3">
        ...
        <select name="status" class="px-3 py-2 border border-gray-300 dark:border-dark-border rounded-lg text-sm bg-white dark:bg-dark-card dark:text-dark-text focus:ring-2 focus:ring-primary-500">
        ...
        <div class="flex gap-2">
            ...
            <a href="<?= url('students') ?>" class="px-4 py-2 border border-gray-300 dark:border-dark-border rounded-lg text-sm bg-white dark:bg-dark-card dark:text-dark-text hover:bg-gray-50 dark:bg-dark-bg">Clear</a>
        </div>
    </form>
</div>
        ...
        <table class="w-full responsive-table">
            <thead class="bg-gray-50 dark:bg-dark-bg border-b">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-dark-muted uppercase">Admission #</th>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-dark-border">
                <?php if (empty($students)): ?>
                    <tr><td colspan="6" class="px-4 py-8 text-center text-sm text-gray-400 dark:text-gray-500">No students found.</td></tr>
                <?php endif; ?>
                    ...
                    <tr class="hover:bg-gray-50 dark:bg-dark-bg">
                        <td data-label="Student" class="px-4 py-3">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <?php if ($s['phone']): ?>
                                        <p class="text-xs text-gray-500 dark:text-dark-muted"><?= e($s['phone']) ?></p>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </td>
                        <td data-label="Admission #" class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300"><?= e($s['admission_no']) ?></td>
                        <td data-label="Class" class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300"><?= e(($s['class_name'] ?? 'N/A') . ' ' . ($s['section_name'] ?? '')) ?></td>
                        <td data-label="Gender" class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300"><?= e(ucfirst($s['gender'])) ?></td>
                        ...
                        <td data-label="Actions" class="px-4 py-3 text-right">
                            <div class="flex items-center justify-end gap-1">
                                <a href="<?= url('students', 'view', $s['id']) ?>" title="View" class="p-2 text-gray-400 dark:text-gray-500 hover:text-primary-600 rounded">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                </a>
                                <?php if (auth_has_permission('students.edit')): ?>
                                    <a href="<?= url('students', 'edit', $s['id']) ?>" title="Edit" class="p-2 text-gray-400 dark:text-gray-500 hover:text-yellow-600 rounded">
