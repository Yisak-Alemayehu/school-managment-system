...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-dark-text">Announcements</h1>
        <?php if ($canManage): ?>
    ...
    <div class="space-y-4">
        <?php if (empty($announcements['data'])): ?>
            <div class="bg-white dark:bg-dark-card rounded-xl shadow-sm border p-8 text-center text-gray-500 dark:text-dark-muted">
                No announcements to display.
            </div>
        <?php else: ?>
            <?php foreach ($announcements['data'] as $a): ?>
                <div class="bg-white dark:bg-dark-card rounded-xl shadow-sm border p-5 hover:shadow-md transition <?= $a['is_pinned'] ? 'border-l-4 border-l-yellow-400' : '' ?>">
                    <div class="flex items-start justify-between gap-3">
                        <div class="flex-1">
                            <?php if ($a['is_pinned']): ?>
                                <span class="text-xs bg-yellow-100 text-yellow-800 px-2 py-0.5 rounded-full font-medium">Pinned</span>
                            <?php endif; ?>
                            <h2 class="text-lg font-semibold text-gray-900 dark:text-dark-text mt-1">
                                <a href="<?= url('communication', 'announcement-view') ?>&id=<?= $a['id'] ?>" class="hover:text-primary-700">
                                    <?= e($a['title']) ?>
                                </a>
                            </h2>
                            <p class="text-sm text-gray-600 dark:text-dark-muted mt-1 line-clamp-2"><?= e(substr(strip_tags($a['content']), 0, 200)) ?></p>
                            <div class="flex items-center gap-4 mt-2 text-xs text-gray-500 dark:text-dark-muted">
                                <span>By <?= e($a['author_name'] ?? 'System') ?></span>
                                <span><?= format_date($a['publish_date']) ?></span>
                                <span class="px-2 py-0.5 rounded-full text-xs <?= $a['target_audience'] === 'all' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 dark:bg-dark-card2 text-gray-700 dark:text-gray-300' ?>">
                                    <?= ucfirst($a['target_audience']) ?>
                                </span>
                                <?php if ($canManage): ?>
                                    <span class="px-2 py-0.5 rounded-full text-xs <?= $a['status'] === 'published' ? 'bg-green-100 text-green-800' : 'bg-gray-100 dark:bg-dark-card2 text-gray-600 dark:text-dark-muted' ?>">
                                        <?= ucfirst($a['status']) ?>
                                    </span>
                                <?php endif; ?>
                            </div>
