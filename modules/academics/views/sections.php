...
<div class="max-w-4xl mx-auto">
    ...
    <h1 class="text-xl font-bold text-gray-900 dark:text-dark-text mb-6">Sections</h1>

    <!-- Form -->
    <div class="bg-white dark:bg-dark-card rounded-xl border border-gray-200 dark:border-dark-border p-6 mb-6">
        <h2 class="text-sm font-semibold text-gray-900 dark:text-dark-text mb-4"><?= $editing ? 'Edit Section' : 'Add New Section' ?></h2>
        <form method="POST" action="<?= url('academics', 'section-save') ?>" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <?= csrf_field() ?>
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Section Name <span class="text-red-500">*</span></label>
            ...
            <div class="flex items-end gap-2">
                ...
                <?php if ($editing): ?>
                    <a href="<?= url('academics', 'sections') ?>" class="px-4 py-2 border border-gray-300 dark:border-dark-border rounded-lg text-sm bg-white dark:bg-dark-card dark:text-dark-text hover:bg-gray-50 dark:bg-dark-bg">Cancel</a>
                <?php endif; ?>
            </div>
        </form>
    </div>
    ...
    <div class="flex items-center gap-3 mb-4">
        <label class="text-sm text-gray-600 dark:text-dark-muted">Filter by Class:</label>
        <select onchange="window.location='<?= url('academics', 'sections') ?>&class_id='+this.value" class="px-3 py-1.5 border border-gray-300 dark:border-dark-border rounded-lg text-sm bg-white dark:bg-dark-card dark:text-dark-text">
        ...
        <div class="overflow-x-auto"><table class="w-full">
            <thead class="bg-gray-50 dark:bg-dark-bg border-b">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-dark-muted uppercase">Capacity</th>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-dark-border">
                <?php if (empty($sections)): ?>
                    <tr><td colspan="5" class="px-4 py-8 text-center text-sm text-gray-500 dark:text-dark-muted">No sections found. Add one above.</td></tr>
                <?php endif; ?>
                <?php foreach ($sections as $s): ?>
                    <tr class="hover:bg-gray-50 dark:bg-dark-bg">
                        ...
                        <td class="px-4 py-3 text-sm text-gray-600 dark:text-dark-muted"><?= $s['capacity'] ?></td>
                        <td class="px-4 py-3 text-sm text-gray-600 dark:text-dark-muted"><?= $s['student_count'] ?></td>
                        <td class="px-4 py-3 text-right">
                            <a href="<?= url('academics', 'sections') ?>&edit=<?= $s['id'] ?>" class="p-2 text-gray-400 dark:text-gray-500 hover:text-yellow-600 rounded inline-block" title="Edit">
