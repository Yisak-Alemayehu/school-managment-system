...
<div class="max-w-4xl mx-auto">
    ...
    <h1 class="text-xl font-bold text-gray-900 dark:text-dark-text mb-6">Streams (Academic Tracks)</h1>

    <!-- Form -->
    <div class="bg-white dark:bg-dark-card rounded-xl border border-gray-200 dark:border-dark-border p-6 mb-6">
        <h2 class="text-sm font-semibold text-gray-900 dark:text-dark-text mb-4"><?= $editing ? 'Edit Stream' : 'Add New Stream' ?></h2>
        <form method="POST" action="<?= url('academics', 'stream-save') ?>" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            <?= csrf_field() ?>
            <?php if ($editing): ?><input type="hidden" name="id" value="<?= $editing['id'] ?>"><?php endif; ?>

            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Stream Name <span class="text-red-500">*</span></label>
            ...
            <div>
                ...
                <select name="is_active" class="w-full px-3 py-2 border border-gray-300 dark:border-dark-border rounded-lg text-sm focus:ring-2 focus:ring-primary-500">
            ...
            <div class="flex items-end gap-2">
                ...
                <?php if ($editing): ?>
                    <a href="<?= url('academics', 'streams') ?>" class="px-4 py-2 border border-gray-300 dark:border-dark-border rounded-lg text-sm hover:bg-gray-50 dark:bg-dark-bg">Cancel</a>
                <?php endif; ?>
            </div>
        </form>
    </div>
        ...
        <div class="overflow-x-auto"><table class="w-full">
            <thead class="bg-gray-50 dark:bg-dark-bg border-b">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-dark-muted uppercase">Stream Name</th>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-dark-border">
                <?php if (empty($streams)): ?>
                    <tr><td colspan="5" class="px-4 py-6 text-center text-sm text-gray-500 dark:text-dark-muted">No streams found. Add one above.</td></tr>
                <?php endif; ?>
                <?php foreach ($streams as $i => $s): ?>
                    <tr class="hover:bg-gray-50 dark:bg-dark-bg">
                        ...
                        <td class="px-4 py-3 text-sm text-gray-600 dark:text-dark-muted"><?= e($s['description'] ?? '—') ?></td>
                        <td class="px-4 py-3">
                            <?php if ($s['is_active']): ?>
                        ...
                        <td class="px-4 py-3 text-right">
                            <a href="<?= url('academics', 'streams') ?>&edit=<?= $s['id'] ?>" class="p-2 text-gray-400 dark:text-gray-500 hover:text-yellow-600 rounded inline-block" title="Edit">
