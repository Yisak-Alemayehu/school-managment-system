...
<div class="max-w-4xl mx-auto">
    ...
    <h1 class="text-xl font-bold text-gray-900 dark:text-dark-text mb-6">Academic Sessions</h1>

    <!-- Add/Edit Form -->
    <div class="bg-white dark:bg-dark-card rounded-xl border border-gray-200 dark:border-dark-border p-6 mb-6">
        <h2 class="text-sm font-semibold text-gray-900 dark:text-dark-text mb-4"><?= $editing ? 'Edit Session' : 'Add New Session' ?></h2>
        <form method="POST" action="<?= url('academics', 'session-save') ?>" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <?= csrf_field() ?>
        ...
        <div class="overflow-x-auto"><table class="w-full">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-dark-border">
                <?php if (empty($sessions)): ?>
                    <tr><td colspan="4" class="px-4 py-8 text-center text-sm text-gray-500 dark:text-dark-muted">No academic sessions found. Add one above.</td></tr>
                <?php endif; ?>
                <?php foreach ($sessions as $s): ?>
                    <tr class="hover:bg-gray-50 dark:bg-dark-bg">
                        ...
                        <td class="px-4 py-3 text-sm text-gray-600 dark:text-dark-muted"><?= format_date($s['start_date']) ?> — <?= format_date($s['end_date']) ?></td>
                        <td class="px-4 py-3">
                            <?php if ($s['is_active']): ?>
                        ...
                        <td class="px-4 py-3 text-right">
                            <div class="flex items-center justify-end gap-1">
                                <a href="<?= url('academics', 'sessions') ?>&edit=<?= $s['id'] ?>" class="p-2 text-gray-400 dark:text-gray-500 hover:text-yellow-600 rounded" title="Edit">
                                ...
                                <form method="POST" action="<?= url('academics', 'session-toggle') ?>" class="inline">
                                    ...
                                    <button type="submit" class="p-2 text-gray-400 dark:text-gray-500 hover:text-primary-600 rounded" title="Toggle active">
