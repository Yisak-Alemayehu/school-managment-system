...
<div class="max-w-5xl mx-auto">
    ...
    <h1 class="text-xl font-bold text-gray-900 dark:text-dark-text mb-6">Assign Subjects to Classes</h1>
        ...
        <div class="bg-white dark:bg-dark-card rounded-xl border border-gray-200 dark:border-dark-border overflow-hidden">
            <div class="px-4 py-3 bg-gray-50 dark:bg-dark-bg border-b flex items-center justify-between">
                <label class="flex items-center gap-2 text-sm">
                    <input type="checkbox" onclick="toggleSelectAll(this, 'subjects[]')" class="rounded border-gray-300 dark:border-dark-border text-primary-600 focus:ring-primary-500">
                    <span class="font-medium text-gray-700 dark:text-gray-300">Select All Subjects</span>
                </label>
                <span class="text-xs text-gray-500 dark:text-dark-muted">Check "Elective" for optional subjects students can choose</span>
            </div>

            <div class="divide-y divide-gray-100 dark:divide-dark-border">
                <?php foreach ($subjects as $s): ?>
                    <div class="flex items-center justify-between px-4 py-3 hover:bg-gray-50 dark:bg-dark-bg">
                        <label class="flex items-center gap-3 cursor-pointer flex-1">
                            ...
                            <div>
                                <span class="text-sm font-medium text-gray-900 dark:text-dark-text"><?= e($s['name']) ?></span>
                                <span class="text-xs text-gray-500 dark:text-dark-muted ml-1">(<?= e($s['code']) ?>)</span>
                            </div>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" name="electives[]" value="<?= $s['id'] ?>"
                                   <?= isset($electiveMap[$s['id']]) ? 'checked' : '' ?>
                                   class="rounded border-gray-300 dark:border-dark-border text-orange-500 focus:ring-orange-400">
                            <span class="text-xs text-gray-500 dark:text-dark-muted">Elective</span>
                        </label>
                    </div>
                <?php endforeach; ?>
            </div>

            <div class="px-4 py-3 bg-gray-50 dark:bg-dark-bg border-t">
