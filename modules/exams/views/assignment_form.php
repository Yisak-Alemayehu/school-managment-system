...
<div class="max-w-2xl mx-auto">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h1 class="text-xl font-bold text-gray-900 dark:text-dark-text"><?= $assignment ? 'Edit Assignment' : 'New Assignment' ?></h1>
    </div>

    <form method="POST" action="<?= url('exams', 'assignment-save') ?>" enctype="multipart/form-data" class="bg-white dark:bg-dark-card rounded-xl border border-gray-200 dark:border-dark-border p-6 space-y-5">
        <?= csrf_field() ?>
        ...
        <div>
            ...
            <input type="file" name="file" class="w-full text-sm text-gray-500 dark:text-dark-muted file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-primary-50 file:text-primary-700 hover:file:bg-primary-100">
            <?php if (!empty($assignment['file_path'])): ?>
                <p class="text-xs text-gray-500 dark:text-dark-muted mt-1">Current file: <?= e(basename($assignment['file_path'])) ?></p>
            <?php endif; ?>
        </div>
