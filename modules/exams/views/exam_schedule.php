...
<div class="max-w-5xl mx-auto">
    <div class="flex items-center gap-3 mb-6">
        <a href="<?= url('exams', 'exams') ?>" class="p-2 hover:bg-gray-100 dark:hover:bg-dark-card2 rounded-lg">
        ...
        <h1 class="text-xl font-bold text-gray-900 dark:text-dark-text">Schedule: <?= e($exam['name']) ?></h1>
    </div>
    ...
    <div class="bg-white dark:bg-dark-card rounded-xl border border-gray-200 dark:border-dark-border p-6 mb-6">
        <h2 class="text-sm font-semibold text-gray-900 dark:text-dark-text mb-4"><?= $editing ? 'Edit Schedule' : 'Add Schedule Entry' ?></h2>
        <form method="POST" action="<?= url('exams', 'exam-schedule-save') ?>" class="grid grid-cols-1 sm:grid-cols-6 gap-4">
            ...
            <div>
                ...
                <select name="class_id" required class="w-full px-3 py-2 border border-gray-300 dark:border-dark-border rounded-lg text-sm bg-white dark:bg-dark-card dark:text-dark-text">
            ...
            <div>
                ...
                <select name="subject_id" required class="w-full px-3 py-2 border border-gray-300 dark:border-dark-border rounded-lg text-sm bg-white dark:bg-dark-card dark:text-dark-text">
                    <option value="">Select</option>
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Full Marks</label>
            ...
            <div class="flex items-end gap-2">
                ...
                <?php if ($editing): ?>
                    <a href="<?= url('exams', 'exam-schedule') ?>&exam_id=<?= $examId ?>" class="px-3 py-2 border border-gray-300 dark:border-dark-border rounded-lg text-sm bg-white dark:bg-dark-card dark:text-dark-text hover:bg-gray-50 dark:bg-dark-bg">Cancel</a>
                <?php endif; ?>
            </div>
        </form>
    </div>
        ...
        <div class="overflow-x-auto"><table class="w-full">
            <thead class="bg-gray-50 dark:bg-dark-bg border-b">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-dark-muted uppercase">Full Marks</th>
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-dark-muted uppercase">Pass</th>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-dark-border">
                <?php if (empty($schedules)): ?>
                    <tr><td colspan="7" class="px-4 py-6 text-center text-sm text-gray-500 dark:text-dark-muted">No schedule entries yet.</td></tr>
                <?php endif; ?>
                <?php foreach ($schedules as $s): ?>
                    <tr class="hover:bg-gray-50 dark:bg-dark-bg">
                        <td class="px-4 py-3 text-sm text-gray-900 dark:text-dark-text"><?= format_date($s['exam_date']) ?></td>
                        <td class="px-4 py-3 text-sm text-gray-600 dark:text-dark-muted"><?= substr($s['start_time'], 0, 5) ?> — <?= substr($s['end_time'], 0, 5) ?></td>
                        <td class="px-4 py-3 text-sm text-gray-600 dark:text-dark-muted"><?= e($s['class_name']) ?></td>
                        <td class="px-4 py-3 text-sm font-medium text-gray-900 dark:text-dark-text"><?= e($s['subject_name']) ?> (<?= e($s['subject_code']) ?>)</td>
                        <td class="px-4 py-3 text-sm text-gray-600 dark:text-dark-muted"><?= $s['full_marks'] ?></td>
                        <td class="px-4 py-3 text-sm text-gray-600 dark:text-dark-muted"><?= $s['pass_marks'] ?></td>
                        <td class="px-4 py-3 text-right">
                            <?php if (auth_has_permission('exam.manage')): ?>
                                <a href="<?= url('exams', 'exam-schedule') ?>&exam_id=<?= $examId ?>&edit=<?= $s['id'] ?>" class="p-1 text-gray-400 dark:text-gray-500 hover:text-yellow-600">
