...
<div class="max-w-4xl mx-auto space-y-6">
    <h1 class="text-2xl font-bold text-gray-900 dark:text-dark-text">System Settings</h1>

    <form method="POST" action="<?= url('settings', 'general-save') ?>">
        <?= csrf_field() ?>

        <?php foreach ($groups as $group => $items): ?>
            <div class="bg-white dark:bg-dark-card rounded-xl shadow-sm border p-6 mb-6">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-dark-text mb-4 capitalize"><?= e(str_replace('_', ' ', $group)) ?> Settings</h2>
                <div class="space-y-4">
                    <?php foreach ($items as $s): ?>
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-start">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300"><?= e(ucwords(str_replace('_', ' ', $s['setting_key']))) ?></label>
                                <?php if ($s['description']): ?>
                                    <p class="text-xs text-gray-500 dark:text-dark-muted"><?= e($s['description']) ?></p>
                                <?php endif; ?>
                            </div>
