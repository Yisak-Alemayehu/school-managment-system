...
<div class="space-y-4">
    <div class="flex items-center justify-between flex-wrap gap-2">
        <h1 class="text-xl font-bold text-gray-900">Fee / Tuition Management</h1>
    ...
    <!-- Search -->
    <form method="GET" action="<?= url('finance', 'fee-due') ?>" class="bg-white rounded-xl border border-gray-200 p-4">
        <div class="flex gap-3">
            ...
            <a href="<?= url('finance', 'fee-due') ?>" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg text-sm hover:bg-gray-200 font-medium">Clear</a>
        </div>
    </form>

    <!-- ═══════════════ ADD NEW FEE FORM ═══════════════ -->
    <div id="addFeeSection" class="hidden bg-white rounded-xl border border-gray-200 p-6">
        <h2 class="text-lg font-bold text-gray-900 mb-4">Add New Fee</h2>
        <form method="POST" action="<?= url('finance', 'fee-save') ?>" id="feeForm">
            <?= csrf_field() ?>

            <!-- Section 1: Basic Information -->
            <div class="border-b border-gray-200 pb-4 mb-4">
                <h3 class="text-sm font-semibold text-gray-700 uppercase mb-3">Basic Fee Information</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div id="foreignAmountWrap" class="hidden">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Exchange Rate Amount (ETB)</label>
            ...
            <div class="border-b border-gray-200 pb-4 mb-4">
                <h3 class="text-sm font-semibold text-gray-700 uppercase mb-3">Fee Validity Period</h3>
            ...
            <div id="recurrentSection" class="border-b border-gray-200 pb-4 mb-4">
                <h3 class="text-sm font-semibold text-gray-700 uppercase mb-3">Recurrent Fee Settings</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Apply Every</label>
            ...
            <div class="border-b border-gray-200 pb-4 mb-4">
                <h3 class="text-sm font-semibold text-gray-700 uppercase mb-3">Penalty Settings</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="flex items-center gap-2 text-sm">
                            <input type="checkbox" name="has_penalty" id="hasPenalty" value="1" checked
                                   onchange="togglePenaltySection()" class="rounded border-gray-300 text-primary-600 focus:ring-primary-500">
                            Has Penalty
                        </label>
                    </div>
                    <div>
            ...
            <div id="penaltyDetail" class="pb-4 mb-4">
                <h3 class="text-sm font-semibold text-gray-700 uppercase mb-3">Arrears / Penalty Detail</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Unpaid After</label>
                        <div class="flex gap-2">
                    ...
                    <div id="fixedPenaltyValue">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Penalty Value</label>
                    ...
                    <div id="penaltyReapplyWrap" class="hidden">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Reapply Every</label>
                        <div class="flex gap-2">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Penalty Expiry Date</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Max Penalty Count</label>
                ...
                <div id="varyingPenaltyWrap" class="hidden mt-4">
                    <h4 class="text-sm font-medium text-gray-700 mb-2">Varying Penalty Values</h4>
                    ...
                    <button type="button" onclick="addVaryingRow()"
                            class="mt-2 px-3 py-1 bg-gray-100 text-gray-700 rounded-lg text-sm hover:bg-gray-200">
                        + Add Value
                    </button>
                </div>
            </div>
            ...
            <div class="flex gap-3">
                ...
                <button type="reset" onclick="document.getElementById('addFeeSection').classList.add('hidden')"
                        class="px-6 py-2 bg-gray-100 text-gray-700 rounded-lg text-sm hover:bg-gray-200 font-medium">Cancel</button>
            </div>
        </form>
    </div>
            ...
            <table class="min-w-full divide-y divide-gray-200 responsive-table">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Classes</th>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Penalty</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    <?php if (empty($fees)): ?>
                    <tr><td colspan="9" class="px-4 py-8 text-center text-gray-400">No fees defined.</td></tr>
                    <?php else: ?>
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-4 py-3 text-sm text-gray-600" data-label="Classes"><?= e($f['class_names'] ?? 'All') ?></td>
                        <td class="px-4 py-3 text-sm text-gray-600" data-label="Students"><?= (int) $f['student_count'] ?></td>
                        <td class="px-4 py-3 text-sm text-gray-600" data-label="Period">
...
function addVaryingRow() {
    ...
    row.innerHTML = '<span class="text-sm text-gray-500 w-8">' + count + '.</span>' +
        '<input type="number" name="varying_values[]" step="0.01" min="0" class="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500" placeholder="Value">' +
        '<button type="button" onclick="removeVaryingRow(this)" class="text-red-500 hover:text-red-700 text-sm">Remove</button>';
