...
<div class="space-y-4">
    <div class="flex items-center justify-between">
        <h1 class="text-xl font-bold text-gray-900">Finance — Grouping</h1>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 responsive-table">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Date Created</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Group Name</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    <?php if (empty($groups)): ?>
                    <tr><td colspan="5" class="px-4 py-8 text-center text-gray-400">No groups created yet.</td></tr>
                    <?php else: foreach ($groups as $g): ?>
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-4 py-3 text-sm text-gray-500" data-label="Date Created"><?= format_date($g['created_at']) ?></td>
                        ...
                        <td class="px-4 py-3 text-sm text-gray-600" data-label="Description"><?= e(truncate($g['description'] ?? '', 60)) ?></td>
                        <td class="px-4 py-3 text-sm text-gray-600" data-label="Members"><?= $g['member_count'] ?></td>
                        <td class="px-4 py-3 text-sm" data-label="Active">
                            <span class="px-2 py-0.5 text-xs font-semibold rounded-full <?= $g['is_active'] ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500' ?>">
                                <?= $g['is_active'] ? 'Yes' : 'No' ?>
                            </span>
                        </td>
                    </tr>
...
<div id="newGroupModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/40">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 p-6">
        <h3 class="text-lg font-bold text-gray-900 mb-4">Add New Group</h3>
        <form method="POST" action="<?= url('finance', 'group-save') ?>">
            ...
            <div class="space-y-4">
                ...
                <div id="classSelectRow" class="hidden">
                    ...
                    <select name="source_class_id" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500">
                ...
                <div>
                    ...
                    <input type="text" name="name" required class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" placeholder="Group Name">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Group Description *</label>
                    <textarea name="description" required rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" placeholder="Group Description"></textarea>
                </div>
                <div class="flex items-center gap-2">
                    <input type="checkbox" name="is_active" value="1" checked id="grpActive" class="rounded">
                    <label for="grpActive" class="text-sm text-gray-700">Active</label>
                </div>
            </div>
            <div class="flex justify-end gap-3 mt-6">
                <button type="button" onclick="this.closest('#newGroupModal').classList.add('hidden')" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg text-sm hover:bg-gray-200">Cancel</button>
