...
- **Internal Code Name:** `school_management`
- **Version:** 1.0.0
- **Database Identifier:** `school_management`
- **Session Identifier:** `school_session`

## 1.2 Project Description

The School Management System is a comprehensive, web-based school administration platform designed for K-12 educational institutions. The system provides end-to-end management of academic operations, student lifecycle, financial transactions, examination & assessment processes, attendance tracking, and internal communication.
...
The platform integrates with payment gateways for online fee collection, supports standard academic calendars and grading standards, and uses configurable currency and timezone settings for localization.
...
| OBJ-03 | Implement examination and assessment tracking with configurable grade scales | All marks, assessments, and report cards generated digitally |
| OBJ-04 | Enable online and offline fee management and payment collection | All fee structures, invoices, and payments tracked; payment gateway integration active |
...
| 3 | **Academic Management** | Academic sessions, terms/semesters, classes (Grade 1-12), sections (A, B, etc.), subjects, mediums (language of instruction), streams (categorization), shifts (Morning/Afternoon) |
...
| 5 | **Examination & Assessment** | Exam creation, exam scheduling, marks entry, assessment management (total marks capped at 100 per subject), student conduct grades (A-F behavioral), grade scales (configurable A+ through F), report card generation (printable A4 with QR), result analysis, class roster |
...
| 9 | **Payment Gateway Integration** | Configurable payment gateway processing with webhook handling, transaction tracking with idempotency, payment reconciliation |
...
| 8 | Advanced payment gateway options | Configuration templates available; market-specific implementation required |
...
| ID | Assumption |
|----|-----------|
| A-01 | The school operates on a standard academic session calendar divided into terms/semesters |
| A-02 | The school follows a configurable grading standard (e.g., A+ through F) |
...
| A-06 | A default currency is configured for the institution |
...
| A-09 | The school has a maximum of 12 grade levels with 2 or more sections each |
| A-10 | A payment gateway is configured for online transactions |
...
| C-02 | **Single-server deployment** — Designed for shared hosting | No horizontal scaling; no load balancing; single MySQL instance |
...
| C-04 | **Configurable payment gateways** — Multiple integrations possible | Payment gateway implementation varies by market |
...
The School Management System uses a **Monolithic Modular Architecture** with a **Front Controller Pattern**. All HTTP requests are routed through a single entry point (`public/index.php`), which bootstraps the application, initializes sessions, and dispatches requests to the appropriate module based on URL parameters.
...
└── gateways/
     └── (gateway implementations - market-specific)
```
...
| `config/payment.php` | 12 | Payment gateway configurations (market-specific), transaction states, timing settings |
...
│  │  Required Extensions:                                │   │
│  │  - pdo_mysql                                         │   │
│  │  - openssl (for payment gateway encryption)          │   │
│  │  - mbstring                                          │   │
...
│  ┌──────────────────────────────────────────────────────┐   │
│  │           Cron Jobs                                  │   │
│  │  cron/penalty_job.php       ← Daily penalty calc    │   │
│  │  cron/recurrence_job.php    ← Monthly fee gen       │   │
│  └──────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
            │                              │
...
POST /finance?action=payment-initiate
     ...
     ├── [Gateway Path]
     │   ├── gateway_initiate()
     │   ├── Build payment params with gateway authentication
     │   ├── POST to Gateway API
...
| **OpenSSL Extension** | — | Encryption for payment gateway communications |
...
|---------|---------|-------------------|
| **Payment Gateway** | Online payment processing | REST API + encryption + webhooks |
...
Payment processing is abstracted through a common interface (`gateway_create_transaction()`, `gateway_update_transaction()`, `gateway_confirm_payment()`) with concrete implementations for each gateway.
